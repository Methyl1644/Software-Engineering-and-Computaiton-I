...
public class Evaluator {
    ...
    static {
        ...
        GLOBALS.define(new LSymbol("atom"), new LFunction( list -> new LAtom(list.content.get(1))));
        ...
        GLOBALS.define(new LSymbol("atom?"), new LFunction(list -> {
            if (list.content.get(1) instanceof LAtom) return new LInteger(1);
            else return new LInteger(0);
        }));
...
// ── TODO 5: deref 函数 ───────────────────────
        ...
        GLOBALS.define(new LSymbol("list"), new LFunction(list -> {
            return new LList(list.content.subList(1, list.content.size()));
        }));
